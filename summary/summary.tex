...
number of graphs.
The log posterior joint distribution $\log p(\Omega_k, \delta_k, \theta_k, z_k \mid
	y_k)$ is equal to
\begin{multline}\label{eq:mg-joint}
	\text{constant} + \frac{1}{2} \log\det(\Omega_k) -\frac{1}{2} \tr(y_k^t y_k \Omega) - \sum_{i<j} \frac{\omega_{ijk}}{2} \frac{1}{v_{\delta_{ijk}}} + \lambda_k \sum_i \omega_{iik} \\
	+ \sum_{i < j} \left\{ \1\{z_{ijk} > 0\}
	\log\left(\frac{\delta_{ijk}}{1 - \delta_{ijk}}\right)
	+ \log(1 - \delta_{ijk}) \right\}
	- \sum_{i < j} \frac{(z_{ijk} - \theta_{ijk})^2}{2}
	+ \log p(\theta_k).
\end{multline}
When we take the conditional expectation of \eqref{eq:mg-joint} over the latent
variables, $\delta_k$ and $z_k$, we obtain
\begin{multline}\label{eq:mg-exp}
	\text{constant} +  - \sum_{i<j} \frac{\omega_{ijk}}{2}
	\E_{\cdot \mid \cdot}\left(\frac{1}{\delta_{ijk}v_1^2 + (1 - \delta_{ijk})v_0^2}\right)
	+ \lambda_k \sum_i \omega_{iik} \\
	+ \sum_{i < j} \left[ \E_{\cdot \mid \cdot}\left\{\1\{z_{ijk} > 0\}
		\log\left(\frac{\delta_{ijk}}{1 - \delta_{ijk}}\right) \right\}
		+ \E_{\cdot \mid \cdot}\left\{ \log(1 - \delta_{ijk}) \right\}\right] \\
	- \sum_{i < j} \frac{\E_{\cdot \mid \cdot}\left\{(z_{ijk} - \theta_{ijk})^2\right\}}{2}
	+ \log p(\theta_k) + \frac{1}{2} \log\det(\Omega_k) -\frac{1}{2} \tr(y_k^t y_k \Omega),
\end{multline}
where $\E_{\cdot \mid \cdot}$ refers to the expectation of $\delta_k,$ and $z_k$ conditioned on $\Omega_k, \theta_k,$ and $y_k.$
We now proceed to the computation of the expectation terms.
First, we need $q_{ijk}$ which is the expectation of $\delta_{ijk}.$ It is equal to
\[
	q_{ijk} = \E_{\cdot \mid \cdot}(\delta_{ijk} = 1) =
	\frac{\Phi(\theta_{ijk}) \Nor(\omega_{ijk} \mid 0, v_1^2)}{\Phi(\theta_{ijk}) \Nor(\omega_{ijk} \mid 0, v_1^2) + (1 - \Phi(\theta_{ijk})) \Nor(\omega_{ijk} \mid 0, v_0^2)}.
\]
Then, the first term in \eqref{eq:mg-exp}, $d_{ijk}$ is
\[
	\E_{\cdot \mid \cdot}\left(\frac{1}{\delta_{ijk}v_1^2 + (1 - \delta_{ijk})v_0^2}\right) = \frac{q_{ijk}}{v_1^2} + \frac{1 - q_{ijk}}{v_0^2}
\]
similarly as before.
