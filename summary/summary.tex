...
\end{align*}
and when $\delta_{ijk} = 0$ it is equal to
\[
...
\begin{multline}
	\E_{\cdot \mid \cdot}(z_-) = \frac{1}{C(\omega_-, \theta_-)} \left[\Nor(\omega_- \mid 0, v_1^2) \int_0^\infty z_- \Nor(z_- \mid \theta, 1) dz_- \right. + \\
		\left. \Nor(\omega_- \mid 0, v_0^2) \int_{-\infty}^0 z_- \Nor(z_- \mid \theta, 1) dz_- \right],
\end{multline}
